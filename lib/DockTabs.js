...
const TabLabel = ({ data }) => {
    ...
    const onDragStart = React.useCallback((e) => {
        ...
        e.setData({
            tab: data,
            panelSize: (0, Algorithm_1.getFloatPanelSize)(panelElement, tabGroup),
            tabGroup: data.group,
        }, getDockId());
...
const DockTabs = ({ panelData, onPanelDragEnd, onPanelDragMove, onPanelDragStart, }) => {
    ...
    let items = React.useMemo(() => tabs.map((tab) => ({
        ...
        children: (React.createElement(DockTabPane_1.default, { id: tab.id, key: tab.id, cacheId: tab.id, cached: tab.cached, active: tab.id === activeId, animated: animated, prefixCls: "dock" }, typeof tab.content === "function" ? tab.content(tab) : tab.content)),
