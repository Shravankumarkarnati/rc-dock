...
const AddNewWindowMenu = React.memo(function _AddNewWindowMenu({ children, showWithLeftClick, panelData, }) {
    const { dockMove } = DockData_1.useDockContext();
    return (React.createElement(rc_dropdown_1.default, { prefixCls: "dock-dropdown", overlay: React.createElement(rc_menu_1.default, { onClick: () => dockMove(panelData, null, "new-window") },
            React.createElement(rc_menu_1.MenuItem, null, "New Window")), trigger: showWithLeftClick ? ["contextMenu", "click"] : ["contextMenu"], mouseEnterDelay: 0.1, mouseLeaveDelay: 0.1 }, children));
});
exports.DockTabs = React.memo(function DockTabBase(props) {
    ...
    const items = tabs.map((tab) => {
        ...
        return {
            ...
            style: DockTabPane_1.getStyles(active, tabPaneAnimated),
        };
    });
    return (React.createElement(rc_tabs_1.default, { prefixCls: "dock", more: { icon: moreIcon }, animated: { inkBar: animated, tabPane: tabPaneAnimated }, renderTabBar: renderTabBar, activeKey: activeId, onChange: onTabChange, popupClassName: classnames_1.default(Utils_1.groupClassNames(group)), items: items }));
});
